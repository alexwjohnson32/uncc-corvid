...
class Buses13FederatePlugin(interface.IDeployable):
    @classmethod
    def _get_specific_path(cls) -> pathlib.Path:
        return pathlib.Path("gridlabd", "13-bus")

    def _get_install_path(self, install_root: str) -> pathlib.Path:
        return pathlib.Path(install_root) / "federate" / "gridlabd" / "13bus"
    ...
    def _get_baseline_files(self, install_root: pathlib.Path) -> BaselineFiles:
        return BaselineFiles(
            install_root / "gld_13bus.glm",
            install_root / "gld_13bus.json"
        )

    def _get_model_files(self, model_path: pathlib.Path) -> ModelFiles:
        return ModelFiles(
            model_path.parent / "baseline_13bus.glm",
            model_path / "gld_13bus.glm",
            model_path / "gld_13bus.json"
        )
...
clock {{
    ...
    def deploy(self, json_config: dict, total_time_seconds: float, deploy_root: str, install_root: str) -> None:
        ...
        model_glm_string = self._get_model_glm_string(model_files.baseline_glm_file, input_data.name, total_time_seconds)
    ...
    def get_exec_json(self, model_name: str, deploy_root: str) -> typing.Dict[str, str]:
        ...
        if absolute_working_directory.exists():
            ...
            json_definition["exec"] = "gridlabd.sh gld_13bus.glm"
